...
\section*{Proof of the cluster-length proposition}\label{app:proofcluster}
...
\section*{Proof of the variance proposition}\label{app:proofvariance}
...
\end{equation}
which is the variance decomposition used in the main retained-information argument. Stationarity implies $\mathbb{P}(I_t=1)=p_w$ and $\mathrm{Var}(I_t)=p_w(1-p_w)$.
...
\end{equation}
This proves the local-covariance identity used in the main derivation. Summing over all pairs with the same shift gives
\begin{equation}
\begin{aligned}
\mathrm{Var}(N_w)
&= (T-m+1)p_w(1-p_w) \\
...
\end{equation}
where $R_{\mathrm{long}}$ collects shifts at least $m$. Dropping all nonpositive invalid overlap terms and all nonnegative contributions from $R_{\mathrm{long}}$ yields the lower bound stated in the main text. This completes the proof.
\end{proof}

\section*{Proof of the retained-information theorem}\label{app:prooffisher}
...
\label{eq:appscaled}
\end{equation}
This is the retained-information expectation identity used in the main text.
...
\section*{Proof of the compensation-weight theorem}\label{app:proofgamma}
...
Each tokenizer token $w \in \mathcal{V}$ is mapped to a DNA string (BPE merge or $k$-mer in DNABERT-2). For token $w$ of nucleotide length $m(w)$, the overlap set $S(w)$ and border polynomial $B_w(z)$ are computed on that string. The continuation mass $\widehat{\rho}_w$ follows the Markov continuation formula above with $\widehat{P}$ estimated from a background corpus. The border score is $\beta_w = \widehat{\rho}_w / (1-\widehat{\rho}_w)$ when $\widehat{\rho}_w < 1$.

\paragraph{Controlled metadata score.}
On synthetic border-aware splits, each sample stores a scalar \texttt{border\_score} in CSV metadata aligned with the constructed difficulty of that sample. During training, the score is batch-normalized and passed to the compensation rule in the main text with $\lambda_{\mathrm{comp}}$, $c_{\min}$, and $c_{\max}$ from the YAML config.

\paragraph{External sequence-estimated score.}
When metadata is unavailable, a label-free score is computed from the input sequence (center-window JSD over $k$-mers), then train-quantile normalized to reduce saturation before compensation. The same compensation path is used; only the score source changes.
...
Config field \texttt{compensation\_strength} equals $\lambda_{\mathrm{comp}}$ in the main compensation rule. Fields \texttt{compensation\_clip\_min} and \texttt{compensation\_clip\_max} equal $c_{\min}$ and $c_{\max}$. LoRA \texttt{rank} and \texttt{alpha} are $r$ and the LoRA scaling numerator. The theoretical $\lambda_{\Gamma}$, $\tau$, and $\alpha$ are not exposed as trainer knobs in the current release.
