...
METHODS = [
    ...
    ("metadata_w64_k2_t10_s3", "human_nontata_heldout_w64_k2_t10_s3_seed{seed}_threshold.json", "Metadata-estimated GERM-BO w64_k2_t10_s3"),
]
...
def load_rows():
    ...
    for method, pattern, label in METHODS:
        for seed in SEEDS:
            metrics = load_metric(ROOT / pattern.format(seed=seed))
            rows.append({"method": method, "label": label, "seed": seed, **metrics})
    return rows
...
def paired(rows):
    ...
    for metric in ["test_accuracy", "test_f1"]:
        deltas = [
            by_key[("metadata_w64_k2_t10_s3", seed)][metric] - by_key[("baseline_lora", seed)][metric]
            for seed in SEEDS
        ]
        ci_low, ci_high = bootstrap_ci(deltas)
        out.append(
            {
                "comparison": "metadata_w64_k2_t10_s3_minus_baseline_lora",
                "metric": metric,
...
def write_markdown(summary, paired_rows):
    path = ROOT / "human_nontata_w64_heldout.md"
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Human Non-TATA Promoters W64 Held-Out Confirmation\n\n")
        handle.write(
            ...
            "and validation-threshold tuned test evaluation. The candidate estimator is label-free "
            "`window=64`, `kmer=2`, `top_ratio=0.10`, `score_scale=3.0`.\n\n"
        )
...
def main():
    ...
    write_csv(ROOT / "human_nontata_w64_heldout.csv", rows)
    write_csv(ROOT / "human_nontata_w64_heldout_summary.csv", summary)
    write_csv(ROOT / "human_nontata_w64_heldout_paired.csv", paired_rows)
