...
SPECS = [
    (
        ...
        "splice_kmer_balanced_confirm55_59_lora_attention_output_classifier_seed{seed}_argmax.json",
    ),
    (
        "germ_bo_quantile_q08_12_comp027",
        "GERM-BO quantile [0.8,1.2] comp0.27",
        "splice_kmer_balanced_confirm55_59_germ_bo_quantile_q08_12_comp027_seed{seed}_argmax.json",
    ),
]
...
def main():
    ...
    write_csv(ROOT / "splice_kmer_balanced_confirmation_55_59.csv", rows)
    write_csv(ROOT / "splice_kmer_balanced_confirmation_55_59_summary.csv", summary)
    write_csv(ROOT / "splice_kmer_balanced_confirmation_55_59_paired.csv", paired_rows)

    path = ROOT / "splice_kmer_balanced_confirmation_55_59.md"
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Splice Strict 3-mer-Balanced Confirmation Seeds 55-59\n\n")
        handle.write(
            "Protocol: strict `3-mer-balanced` split, seeds `55-59`, same single-GPU training budget, "
