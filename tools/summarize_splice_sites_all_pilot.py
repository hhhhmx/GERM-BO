...
def load_rows():
    ...
    for method, label in METHODS:
        for seed in SEEDS:
            path = ROOT / f"splice_sites_all_{method}_seed{seed}_argmax.json"
            ...
            rows.append(
                {
                    ...
                    "val_f1": data["validation"]["macro_f1"],
                    "test_accuracy": data["test"]["accuracy"],
                    "test_f1": data["test"]["macro_f1"],
                    "test_precision": "",
                    "test_recall": "",
                }
            )
...
def write_markdown(summary, paired_rows):
    path = ROOT / "splice_sites_all_pilot.md"
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Splice Sites All Boundary-Sensitive Pilot\n\n")
        handle.write(
            "Protocol: Hugging Face `InstaDeepAI/nucleotide_transformer_downstream_tasks/splice_sites_all`, "
            "3-class splice-site classification, pilot subset `3000/600/1200`, seeds `42-44`, "
            "real DNABERT-2 backbone, validation-accuracy best checkpoint, and argmax test evaluation. "
            "Metadata-estimated GERM-BO uses a label-free center-window k-mer JSD score.\n\n"
        )
...
def main():
    ...
    write_csv(ROOT / "splice_sites_all_pilot.csv", rows)
    write_csv(ROOT / "splice_sites_all_pilot_summary.csv", summary)
    write_csv(ROOT / "splice_sites_all_pilot_paired.csv", paired_rows)
