...
RUN_LOG="results/splice_sites_all_pilot_run.log"
STATUS="results/splice_sites_all_pilot_status.tsv"
DONE="results/splice_sites_all_pilot.done"
...
run_one() {
  ...
  local config="$2"
  local seed="$3"
  local run_id="splice_sites_all_${method}_seed${seed}"
...
PYTHONPATH=/home/sy/germ_bo_project ${PYTHON_BIN} tools/summarize_splice_sites_all_pilot.py \
  >> "${RUN_LOG}" 2>&1

date -Iseconds > "${DONE}"
echo "splice_sites_all_pilot_done"
