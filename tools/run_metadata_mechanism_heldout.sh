...
RUN_LOG="results/metadata_mechanism_heldout_run.log"
STATUS="results/metadata_mechanism_heldout_status.tsv"
DONE="results/metadata_mechanism_heldout.done"
...
run_one() {
  local task="$1"
  local seed="$2"
  local config="configs/real_dnabert2_germ_bo_border_${task}_metadata_comp027_p4.yaml"
  local run_id="metadata_${task}_comp027_p4_seed${seed}"
  local outdir="outputs/${run_id}"
  local json_path="results/${run_id}_threshold.json"
  local csv_path="results/${run_id}_predictions.csv"

  if [[ -s "${json_path}" ]]; then
    echo -e "${task}\tmetadata_comp027\t${seed}\tskipped_existing\t${json_path}" | tee -a "${STATUS}"
    return 0
  fi
  ...
  rm -rf "${outdir}"
  echo -e "${task}\tmetadata_comp027\t${seed}\tdone\t${json_path}" | tee -a "${STATUS}"
}
...
date -Iseconds > "${DONE}"
echo "metadata_mechanism_heldout_done"
