...
METHODS = [
    ...
    ("germ_bo_metadata", "GERM-BO metadata-driven"),
]
...
def bootstrap_ci(deltas, iterations=10000, seed=20260419):
...
def load_rows():
    ...
    for method, label in METHODS:
        for seed in SEEDS:
            path = ROOT / f"uci_promoter_{method}_seed{seed}_threshold.json"
            data = json.loads(path.read_text(encoding="utf-8"))
            rows.append(
                {
                    "benchmark": "uci_promoter",
                    "method": method,
                    "label": label,
                    "seed": seed,
...
def write_markdown(summary, paired_rows):
    path = ROOT / "uci_promoter_benchmark_pilot.md"
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Real Benchmark Pilot: UCI Promoter\n\n")
        handle.write(
            "Protocol: UCI Molecular Biology Promoter dataset, real DNABERT-2 backbone, "
            "single GPU, seeds `42-46`, validation-accuracy best checkpoint, and "
            "validation-threshold tuned test evaluation. This is a pilot benchmark, not a final "
...
def main():
    ...
    paired_rows = []
    paired_rows.extend(paired(rows, "germ_bo_metadata", "baseline_lora"))
    paired_rows.extend(paired(rows, "germ_bo_metadata", "germ_bo_activation"))
    paired_rows.extend(paired(rows, "germ_bo_activation", "baseline_lora"))
    write_csv(ROOT / "uci_promoter_benchmark_pilot.csv", rows)
    write_csv(ROOT / "uci_promoter_benchmark_pilot_summary.csv", summary)
    write_csv(ROOT / "uci_promoter_benchmark_pilot_paired.csv", paired_rows)
    md_path = write_markdown(summary, paired_rows)
    print(md_path)
