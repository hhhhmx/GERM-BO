...
RUN_LOG="results/splice_kmer_balanced_confirmation_55_59_run.log"
STATUS="results/splice_kmer_balanced_confirmation_55_59_status.tsv"
DONE="results/splice_kmer_balanced_confirmation_55_59.done"
...
run_one() {
  ...
  local config="$2"
  local seed="$3"
  local run_id="splice_kmer_balanced_confirm55_59_${method}_seed${seed}"
...
for seed in 55 56 57 58 59; do
  run_one "lora_attention_output_classifier" \
    "configs/real_dnabert2_lora_attention_output_classifier_splice_sites_all_kmer_balanced.yaml" \
    "${seed}"
...
PYTHONPATH=/home/sy/germ_bo_project CUDA_VISIBLE_DEVICES=3 ${PYTHON_BIN} tools/summarize_splice_kmer_balanced_confirmation_55_59.py \
  >> "${RUN_LOG}" 2>&1

date -Iseconds > "${DONE}"
echo "splice_kmer_balanced_confirmation_55_59_done"
