...
RUN_LOG="results/nt_histone_marks_pilot_run.log"
STATUS="results/nt_histone_marks_pilot_status.tsv"
DONE="results/nt_histone_marks_pilot.done"
...
PYTHONPATH=/home/sy/germ_bo_project ${PYTHON_BIN} tools/generate_nt_histone_configs.py \
  >> "${RUN_LOG}" 2>&1
...
PYTHONPATH=/home/sy/germ_bo_project ${PYTHON_BIN} tools/summarize_nt_histone_marks_pilot.py \
  >> "${RUN_LOG}" 2>&1

PYTHONPATH=/home/sy/germ_bo_project ${PYTHON_BIN} tools/summarize_nt_downstream_all.py \
  >> "${RUN_LOG}" 2>&1

date -Iseconds > "${DONE}"
echo "nt_histone_marks_pilot_done"
