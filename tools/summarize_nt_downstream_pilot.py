...
SEEDS = [42, 43, 44]
TASKS = [
    ("H3K4me3", "H3K4me3 chromatin mark"),
    ("enhancers", "Enhancers (regulatory elements)"),
]
METHODS = [
                ...
                rows.append(
                    {
                        ...
                        "val_f1": data["validation"].get("macro_f1", data["validation"].get("f1")),
...
def write_markdown(summary, paired_rows):
    path = ROOT / "nt_downstream_pilot.md"
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# NT Downstream Pilot: Chromatin and Regulatory Tasks\n\n")
        handle.write(
            "Protocol: Hugging Face `InstaDeepAI/nucleotide_transformer_downstream_tasks`, "
            "binary classification on `H3K4me3` (histone ChIP-seq peak) and `enhancers`, "
            "pilot subset `2000/500/1000`, seeds `42-44`, real DNABERT-2 backbone, "
            "validation-accuracy best checkpoint, argmax test evaluation. "
            "Metadata-estimated GERM-BO uses label-free center-window k-mer JSD with train-quantile normalization.\n\n"
        )
        for task, task_label in TASKS:
            handle.write(f"## {task_label} (`{task}`)\n\n")
        ...
        handle.write("| Task | Metric | Delta Mean | Bootstrap 95% CI | Win Rate | Per-Seed Delta |\n")
        handle.write("|---|---|---:|---:|---:|---|\n")
        for row in paired_rows:
            handle.write(
                f"| {row['task']} | {row['metric']} | {row['mean_delta']:+.4f} | "
...
def main():
    ...
    write_csv(ROOT / "nt_downstream_pilot.csv", rows)
    write_csv(ROOT / "nt_downstream_pilot_summary.csv", summary)
    write_csv(ROOT / "nt_downstream_pilot_paired.csv", paired_rows)
