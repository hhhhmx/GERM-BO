import json
import statistics
from pathlib import Path


ROOT = Path("results")
SEEDS = [50, 51, 52, 53, 54]

METHOD_SPECS = [
    (
        "NT v2 50m LoRA",
        "nt_v2_50m",
        "cross_backbone_nt_v2_50m_lora_border_hard_seed{seed}_argmax.json",
    ),
    (
        "NT v2 50m GERM-BO metadata comp0.27",
        "nt_v2_50m",
        "cross_backbone_nt_v2_50m_germ_bo_border_hard_seed{seed}_argmax.json",
    ),
    (
        "HyenaDNA tiny LoRA",
        "hyenadna_tiny",
        "cross_backbone_hyenadna_tiny_lora_border_hard_seed{seed}_argmax.json",
    ),
    (
        "HyenaDNA tiny GERM-BO metadata comp0.27",
        "hyenadna_tiny",
        "cross_backbone_hyenadna_tiny_germ_bo_border_hard_seed{seed}_argmax.json",
    ),
]


def mean(values):
    return statistics.mean(values)


def std(values):
    return statistics.stdev(values) if len(values) > 1 else 0.0


def load_rows():
    rows = []
    missing = []
    for label, backbone, pattern in METHOD_SPECS:
        for seed in SEEDS:
            path = ROOT / pattern.format(seed=seed)
            if not path.exists():
                missing.append(str(path))
                continue
            data = json.loads(path.read_text(encoding="utf-8"))
            rows.append(
                {
                    "label": label,
                    "backbone": backbone,
                    "seed": seed,
                    "test_accuracy": data["test"]["accuracy"],
                    "test_macro_f1": data["test"]["macro_f1"],
                }
            )
    return rows, missing


def summarize(rows):
    summary = []
    by_label = {}
    for row in rows:
        by_label.setdefault(row["label"], []).append(row)
    for label, group in by_label.items():
        acc = [row["test_accuracy"] for row in group]
        f1 = [row["test_macro_f1"] for row in group]
        summary.append(
            {
                "label": label,
                "backbone": group[0]["backbone"],
                "seeds": len(group),
                "test_accuracy_mean": mean(acc),
                "test_accuracy_std": std(acc),
                "test_macro_f1_mean": mean(f1),
                "test_macro_f1_std": std(f1),
                "per_seed_acc": " / ".join(f"{value:.4f}" for value in acc),
            }
        )
    return summary


def main():
    rows, missing = load_rows()
    summary = summarize(rows)
    md_path = ROOT / "cross_backbone_border_hard.md"
    lines = [
        "# Cross-Backbone Replication on border_hard (seeds 50--54)",
        "",
        "Protocol: held-out seeds 50--54, oracle metadata border scores, rank 8, lr 3e-4, early stopping patience 4.",
        "",
        "## Summary",
        "",
        "| Method | Seeds | Test Acc Mean +/- Std | Test Macro-F1 Mean +/- Std | Per-Seed Acc |",
        "|---|---:|---:|---:|---|",
    ]
    for row in summary:
        lines.append(
            f"| {row['label']} | {row['seeds']} | "
            f"{row['test_accuracy_mean']:.4f} +/- {row['test_accuracy_std']:.4f} | "
            f"{row['test_macro_f1_mean']:.4f} +/- {row['test_macro_f1_std']:.4f} | "
            f"{row['per_seed_acc']} |"
        )
    if missing:
        lines.extend(["", "## Missing runs", ""])
        lines.extend(f"- `{path}`" for path in missing)
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(md_path)


if __name__ == "__main__":
    main()
