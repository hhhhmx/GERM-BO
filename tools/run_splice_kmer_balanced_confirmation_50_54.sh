...
RUN_LOG="results/splice_kmer_balanced_confirmation_50_54_run.log"
STATUS="results/splice_kmer_balanced_confirmation_50_54_status.tsv"
DONE="results/splice_kmer_balanced_confirmation_50_54.done"
...
run_one() {
  ...
  local config="$2"
  local seed="$3"
  local run_id="splice_kmer_balanced_confirm_${method}_seed${seed}"
...
PYTHONPATH=/home/sy/germ_bo_project CUDA_VISIBLE_DEVICES=3 ${PYTHON_BIN} tools/summarize_splice_kmer_balanced_confirmation_50_54.py \
  >> "${RUN_LOG}" 2>&1

date -Iseconds > "${DONE}"
echo "splice_kmer_balanced_confirmation_50_54_done"
